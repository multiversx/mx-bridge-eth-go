...
const (
	...
	getLastExecutedPeerBatchIDFromMultiversX          = "GetLastExecutedPeerBatchIDFromMultiversX"
...
func createMockBridge(args argsBridgeStub) (*bridgeTests.BridgeExecutorStub, *errorHandler) {
	...
	stub.GetLastExecutedPeerBatchIDFromMultiversXCalled = func(ctx context.Context) (uint64, error) {
		if args.failingStep == getLastExecutedPeerBatchIDFromMultiversX {
			return 0, errHandler.storeAndReturnError(expectedErr)
		}
...
func TestHappyCaseWhenLeader(t *testing.T) {
	...
	assert.Equal(t, 4, executor.GetFunctionCounter(getLastExecutedPeerBatchIDFromMultiversX))
